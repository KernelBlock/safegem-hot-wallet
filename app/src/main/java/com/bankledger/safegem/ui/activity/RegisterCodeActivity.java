...
public class RegisterCodeActivity extends MVPBaseActivity<IRegCodeView, RegCodePresenter> implements IRegCodeView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(R.string.register_str);
    ...
    @OnClick({R.id.agree_tv, R.id.confirm_btn, R.id.register_tv, R.id.get_code_tv, R.id.country_tv})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.country_tv:
                startActivityForResult(new Intent(RegisterCodeActivity.this, CountryCodeActivity.class), Constants.REQUEST_CODE);
                break;
            case R.id.register_tv:
                ActivitySkipUtil.skipAnotherActivityFinish(RegisterCodeActivity.this, LoginActivity.class);
                break;
            case R.id.agree_tv:
                HashMap<String, String> helep_map = new HashMap<>();
                helep_map.put(Constants.INTENT_DATA, getString(R.string.agree_str_title));
                helep_map.put(Constants.URL, getString(R.string.use_agreement_url));
                ActivitySkipUtil.skipAnotherActivity(RegisterCodeActivity.this, WebviewActivity.class, helep_map);
                break;
            case R.id.confirm_btn:
                check();
                break;
            case R.id.get_code_tv:
                if (phoneNum.getText() == null || phoneNum.getText().toString().length() == 0) {
                ...
                mPresenter.checkPhone(Constants.LEFT_BRACKET + countryTv.getText().toString().trim() + Constants.RIGHT_BRACKET + phoneNum.getText().toString());
                break;
        }
    }
    ...
    private void check() {
        ...
        if (inputCode.getText() == null || inputCode.getText().toString().length() == 0) {
            ToastUtil.showShort(this, R.string.code_tip1);
            return;
        }
        if (!check.isChecked()) {
            ToastUtil.showShort(this, R.string.check_tip);
            return;
        }
        if (countryTv.getText().toString().equals(Constants.CHINA_CODE) && !EditValidateUtil.getInstance().isMobileNO(phoneNum.getText().toString())) {
            ToastUtil.showShort(this, R.string.phone_tip2);
    ...
    public void phoneExist() {
        phoneNum.setText("");
        ToastUtil.showShort(this, R.string.phone_registered);
    }
    ...
    public void onValidateCodeSuccess(String content) {
        ...
        ActivitySkipUtil.skipAnotherActivity(RegisterCodeActivity.this, SetPwdActivity.class, map);
    }
